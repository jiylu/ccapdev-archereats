...
import { addToFavoriteRestaurants, createUser, fetchUserById, removeFromFavoriteRestaurants, updateUserById, checkUsernameAvailability, fetchUserByUsername, resetPassword, checkLoginExists } from 'controllers/user.controller.js';

const router = express.Router();

router.post('/createUser', createUser); // url: localhost:8080/api/users/createUser
router.get("/check-username", checkUsernameAvailability);
router.get("/check-login", checkLoginExists);
